	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case 0:
		...
		if (fled_gpio_config != FLED_GPIO_OS) {
			pinctrl = devm_pinctrl_get_select(sm5705_fled->dev->parent, FLED_PINCTRL_STATE_DEFAULT);
			if (IS_ERR(pinctrl))
				pr_err("%s: flash %s pins are not configured\n", __func__, FLED_PINCTRL_STATE_DEFAULT);
		}
		...
		pinctrl = devm_pinctrl_get_select(sm5705_fled->dev->parent, FLED_PINCTRL_STATE_SLEEP);
		if (IS_ERR(pinctrl))
